...
TEST(Integer, minus_digits_2) {
        ...
        const int a[] = {1};
        const int c[] = {1,2,3,4,4};
              int x[10];
        const int* p = minus_digits(a, a + 1, b, b + 5, x);
        ASSERT_EQ(5, p - x );
        ASSERT_TRUE(std::equal(const_cast<const int*>(x), p, c));
    }
...
TEST(Integer, equal_to2) {
    try {
        const Integer<int> x = 22222;
        const Integer<int> y = 22222;
        ASSERT_EQ(      x, y);
        ASSERT_EQ(      x, 22222);
        ASSERT_EQ(  22222, x);}
    catch (std::invalid_argument& e) {
